...
runrfcat=function(Y,indice,lag){
  ...
  pred_probs=predict(model,X.out,type = "response") #generate forecast
    if (pred_probs > 0.5) {    
      pred = cbind(1)  
    } else {  
      pred = cbind(0)  
...
rfcat.rolling.window=function(Y,nprev,indice=1,lag=1){
  ...
  for(i in nprev:1){#NB: backwards FOR loop: going from 180 down to 1
    ...
    rf=runrfcat(Y.window,indice,lag)#call the function to fit the Random Forest and generate h-step forecast
