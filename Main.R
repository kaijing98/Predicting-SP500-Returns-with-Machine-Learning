...
setwd("/Users/Kaijing/documents/ec4308/Empirical-Performance-of-Equity-Premium-Prediction-on-Various-Machine-Learning")
source("func-ann.R")
source("func-lasso.R")
...
#read data 
#########################################################
#read the file: df and df_cat are the original data file for regressors, and actual returns and directions respectively
...
yy = df$SP500R
#test data between 2015 to 2019 (test window)
nprev = 60 #test set size i.e. 60 months
oosy = tail(yy,nprev)

##New Data: to predict direction of return (0: negative return; 1: positive/no change in return)
#drop original SP500R variable so it doesn't get passed in as a predictor
...
## inspect data
###############################################################################
adf.test(yy, nlag = 4, output = TRUE)
...
## Elastic Net
###############################################################################
alpha=0.5 #set alpha=0.5 for Elastic Net
...
## Ridge-regression
###############################################################################
alpha=0 #set alpha=0 for RIDGE

#Run forecasts for Ridge (BIC)
#note in this case indice is set to 15 as SP500R is the 15th column
...
calc_accuracy(boosted1ac$pred, actual_direction) 
calc_accuracy(boosted3ac$pred, actual_direction) #0.55
calc_accuracy(boosted6ac$pred, actual_direction) #0.6666667
calc_accuracy(boosted12ac$pred, actual_direction) #0.6666667

###############################################################################
###Transforming prediction to direction
###############################################################################
#Lasso
...
boosted1a_dir = get_direction(boosted1a$pred)
boosted3a_dir = get_direction(boosted3a$pred)
boosted6a_dir = get_direction(boosted6a$pred)
boosted12a_dir = get_direction(boosted12a$pred)
...
#See the MSE:
rf.mse1=rf1c$errors[1] # MSE = 13.29019 for 1 lag
rf.mse1
rf.mse3=rf3c$errors[1] # MSE = 14.41212 for 3 lags
rf.mse3
rf.mse6=rf6c$errors[1] # MSE = 13.82297  for 6 lags
rf.mse6
rf.mse12=rf12c$errors[1] # MSE = 13.44896 for 12 lags
rf.mse12
...
calc_accuracy(get_direction(rf1c$pred),actual_direction) #0.5833333
calc_accuracy(get_direction(rf3c$pred),actual_direction) #0.6
calc_accuracy(get_direction(rf6c$pred),actual_direction) #0.5666667
calc_accuracy(get_direction(rf12c$pred),actual_direction) #0.65

#categorical analysis using random forest
set.seed(12345)
#Rolling window forecast
...
calc_accuracy(rf1cc$pred, actual_direction) #0.7
calc_accuracy(rf3cc$pred, actual_direction) #0.5833333
calc_accuracy(rf6cc$pred, actual_direction) #0.6166667
calc_accuracy(rf12cc$pred, actual_direction) #0.65
...
pred1a.mean = forecast(mean1$model, tail(Y, 60), 1, 1)
pred1a.mean
pred3a.mean = forecast(rf3a$model, tail(Y, 60), 1, 3, 1)
pred3a.mean
pred6a.mean = forecast(rf6a$model, tail(Y, 60), 1, 6, 1)
pred6a.mean
pred12a.mean = forecast(rf12a$model, tail(Y, 60), 1, 12, 1)
pred12a.mean

mean1a_dir = get_direction(mean1$pred)
mean3a_dir = get_direction(mean3$pred)
mean6a_dir = get_direction(mean6$pred)
mean12a_dir = get_direction(mean12$pred)

calc_accuracy(mean1a_dir, actual_direction) 
calc_accuracy(mean3a_dir, actual_direction)
calc_accuracy(mean6a_dir, actual_direction) 
calc_accuracy(mean12a_dir, actual_direction) 
...
x1 = model.matrix(SP500R~.-1, data = train) #training data, -1 to remove intercept
x2 = model.matrix(SP500R~.-1, data = vald) #validation data
x3 = model.matrix(SP500R~.-1, data = test) #test data
y1=train$SP500R #y for training data
y2=vald$SP500R  #y for validation data
y3=test$SP500R  #y for test data

#Fit a single pruned tree first using default settings:
big.tree = rpart(SP500R~.,method="anova",data=train) #big tree
...
tic()
boost.fit = gbm(SP500R~.,data=train,distribution='gaussian',interaction.depth=5,n.trees=10000,shrinkage=.01,cv.folds=10)
...
tic()
boost.fit2 = gbm(SP500R~.,data=train,distribution='gaussian',interaction.depth=2,n.trees=10000,shrinkage=.01,cv.folds=10)
...
###############################################################################
# SECOND PART: Loss Differentials
###############################################################################
